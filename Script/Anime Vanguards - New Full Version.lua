...
if game.PlaceId == 16146832113 then
    ...
    local function Manager_Upgrade(TexT, Name)
        if OwnGui.UnitManager.Holder.List[TexT].UpgradeLabel.Text == "Upgrade (MAX)" then
            return Unit_Data(Name).upgradeprice[#Unit_Data(Name).upgradeprice].Price
        else
            local num = OwnGui.UnitManager.Holder.List[TexT].UpgradeLabel.Text:split(" ")[2]:gsub("%[",""):gsub("%]","")
            return Unit_Data(Name).upgradeprice[tonumber(num) + 1].Price
        end
    end
        ...
        function()
            Game.Signals.Upgrade = OwnGui.UpgradeInterfaces.ChildAdded:Connect(
                function(v)
                    ...
                    local Buttons_Button = v:WaitForChild("Buttons")
                    if not Game.Signals[v.Name.."Upgrade"] then
                        Game.Signals[v.Name.."Upgrade"] = Upgrade_Button:GetPropertyChangedSignal("Text"):Connect(
                            function()
                                if Configs["Macro Record"].Value and not Loader.Unloaded and Units_Active(v.Name) then
                                    local unit, last_macro = Units_Active(v.Name), Macro.Value[tostring(Macro_Len())]

                                    Macro_Data_Write()
                                    if last_macro["type"] == "Upgrade" and last_macro["unit"] == unit.name and last_macro["value"] == tostring(unit.current_upgrade - 1) and last_macro["money"] == tostring(Money_Write(unit.name, "Upgrade")) and last_macro["cframe"] == tostring(Unit_CFrame(v.Name)) then
                                    else
                                        Macro_Insert(
                                            {
                                                ["type"] = "Upgrade",
                    ...
                    if not Game.Signals[v.Name.."Priority"] then
                    ...
                    if not Game.Signals[v.Name.."Sell"] then
                        Game.Signals[v.Name.."Sell"] = Sell_Button.InputBegan:Connect(
                            function(input)
                                if (input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch) and Configs["Macro Record"].Value and not Loader.Unloaded and Units_Active(v.Name) then
                                    local unit, last_macro = Units_Active(v.Name), Macro.Value[tostring(Macro_Len())]
                                    Macro_Data_Write()

                                    if last_macro["type"] == "Sell" and last_macro["unit"] == unit.name and last_macro["cframe"] == tostring(Unit_CFrame(v.Name)) then
                                    else
                                        Macro_Insert(
                                            {
                                                ["type"] = "Sell",
                    ...
                    if Buttons_Button:FindFirstChild("AbilityIcon") then
                        if not Game.Signals[v.Name.."AbilityButton"] then
                            Game.Signals[v.Name.."AbilityButton"] = Buttons_Button.AbilityIcon.Button.InputBegan:Connect(
                                function(input)
                                    if (input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch) and Configs["Macro Record"].Value and not Loader.Unloaded and Units_Active(v.Name) then
                                        if Buttons_Button.AbilityIcon:FindFirstChild(v.Name) and Buttons_Button.AbilityIcon[v.Name]:FindFirstChild("Cooldown") then
                                        else
                                            local unit = Units_Active(v.Name)
                                            ...
                                            Macro_Insert(
                                                {
                                                    ["type"] = "Activate",
                                                    ["unit"] = unit.name,
                                                    ["value"] = v.Buttons.AbilityIcon.Main.Label.Text,
                                                    ["money"] = "0",
                                                    ["time"] = tostring(Time_Write()),
                                                    ["cframe"] = tostring(Unit_CFrame(v.Name))
                                                }
                        ...
                        if not Game.Signals[v.Name.."AbilityAutoButton"] then
                            Game.Signals[v.Name.."AbilityAutoButton"] = Buttons_Button.AbilityIcon.AutoUse.Ball:GetPropertyChangedSignal("BackgroundColor3"):Content(
                                function()
                                    if Configs["Macro Record"].Value and not Loader.Unloaded and Units_Active(v.Name) then
                                        local unit = Units_Active(v.Name)
                    ...
                    task.spawn(
                        function()
                            ...
                            if Game.Signals[v.Name.."AbilityButton"] then
                               Game.Signals[v.Name.."AbilityButton"]:Disconnect()
                               Game.Signals[v.Name.."AbilityButton"] = nil
                            end
                            if Game.Signals[v.Name.."AbilityAutoButton"] then
                               Game.Signals[v.Name.."AbilityAutoButton"]:Disconnect()
                               Game.Signals[v.Name.."AbilityAutoButton"] = nil
                            end
                        end
                    )
                end
    ...
    task.spawn(
        function()
            OwnGui.ChildAdded:Connect(
                function(v)
                    if v.Name == "UnitManager" then
                        local List = v:WaitForChild("Holder"):WaitForChild("List")
                        local Units = {}
                        for I, V in next, List:GetChildren() do
                            if V:IsA("Frame") and not Units[V.Name] then
                                ...
                                Game.Units_Manager[V.Name.."Upgrade"] = V.UpgradeLabel:GetPropertyChangedSignal("Text"):Connect(
                                 function()
                                    if Configs["Macro Record"].Value and not Loader.Unloaded and Units_Active(V.Name) then
                                        ...
                                        Macro_Data_Write()
                                        if last_macro["type"] == "Upgrade" and last_macro["unit"] == unit.name and last_macro["value"] == tostring(unit.current_upgrade - 1) and last_macro["money"] == tostring(Money_Write(V.Name, unit.name)) and last_macro["cframe"] == tostring(Unit_CFrame(V.Name)) then
                                        else
                                            Macro_Insert(
                                                {
                                                    ["type"] = "Upgrade",
                                                    ["unit"] = unit.name,
                                                    ["value"] = tostring(unit.current_upgrade - 1),
                                                    ["money"] = tostring(Money_Write(V.Name, unit.name)),
                                                    ["time"] = tostring(Time_Write()),
                                                    ["cframe"] = tostring(Unit_CFrame(V.Name))
                                                }
                                            )
                        ...
                        if not Game.Units_Manager["Sell"] then
                            Game.Units_Manager["Sell"] = List.ChildRemoved:Connect(
                                function(x)
                                    if Configs["Macro Record"].Value and not Loader.Unloaded and Units[x.Name] then
                        ...
                        if not Game.Units_Manager["ChildAdded"] then
                            Game.Units_Manager["ChildAdded"] = List.ChildAdded:Connect(
                                function(x)
                                    if x:IsA("Frame") and not Units[x.Name] then
                                        Units[x.Name] = {Data = Units_Active(x.Name), Position = tostring(Unit_CFrame(x.Name))}
                                    end
                                    if x:IsA("Frame") and not Game.Units_Manager[x.Name.."Upgrade"] then
                                        Game.Units_Manager[x.Name.."Upgrade"] = x:WaitForChild("UpgradeLabel"):GetPropertyChangedSignal("Text"):Connect(
                                            function()
                                                if Configs["Macro Record"].Value and not Loader.Unloaded and Units_Active(x.Name) then
                                                    local unit, last_macro = Units_Active(x.Name), Macro.Value[tostring(Macro_Len())]

                                                    Macro_Data_Write()
                                                    if last_macro["type"] == "Upgrade" and last_macro["unit"] == unit.name and last_macro["value"] == tostring(unit.current_upgrade - 1) and last_macro["money"] == tostring(Money_Write(x.Name, unit.name)) and last_macro["cframe"] == tostring(Unit_CFrame(x.Name)) then
                                                    else
                                                        Macro_Insert(
                                                            {
                                                                ["type"] = "Upgrade",
                                                                ["unit"] = unit.name,
                                                                ["value"] = tostring(unit.current_upgrade - 1),
                                                                ["money"] = tostring(Money_Write(x.Name, unit.name)),
                                                                ["time"] = tostring(Time_Write()),
                                                                ["cframe"] = tostring(Unit_CFrame(x.Name))
                                                            }
                                                        )
                                                    end
                                                    Macro_Write()
                        ...
                        task.spawn(
                            function()
                                repeat task.wait() until not v.Parent or Loader.Unloaded
                                if Game.Units_Manager["Sell"] then
                                   Game.Units_Manager["Sell"]:Disconnect()
                                   Game.Units_Manager["Sell"] = nil
                                end
                                if Game.Units_Manager["ChildAdded"] then
                                   Game.Units_Manager["ChildAdded"]:Disconnect()
                                   Game.Units_Manager["ChildAdded"] = nil
                                end
                                for i = 1,#Game.Units_Manager do
                                    for i,v in pairs(Game.Units_Manager[i]) do
                                        if v then
                                           v:Disconnect()
                                           v = nil
                                        end
                                ...
                                for i,v in pairs(Game.Units_Manager) do
                                    if type(v) == "userdata" then
                                        if Game.Units_Manager[v] ~= nil then
                                            Game.Units_Manager[v]:Disconnect()
                                            Game.Units_Manager[v] = nil
                                        end
                                    end
                                end
                            end
            ...
            if OwnGui:FindFirstChild("UnitManager") then
                local List = OwnGui.UnitManager:WaitForChild("Holder"):WaitForChild("List")
                local Units = {}
                for I, V in next, List:GetChildren() do
                    if V:IsA("Frame") and not Units[V.Name] then
                        Units[V.Name] = {Data = Units_Active(V.Name), Position = tostring(Unit_CFrame(V.Name))}
                        ...
                        Game.Signals[UpgradeInterface.Name.."Upgrade"] = UpgradeInterface:WaitForChild("Stats"):WaitForChild("UpgradeButton"):WaitForChild("Inner"):WaitForChild("Label"):GetPropertyChangedSignal("Text"):Connect(
                            function()
                                if Configs["Macro Record"].Value and not Loader.Unloaded and Units_Active(UpgradeInterface.Name) then
                                    local unit, last_macro = Units_Active(UpgradeInterface.Name), Macro.Value[tostring(Macro_Len())]

                                    Macro_Data_Write()
                                    if last_macro["type"] == "Upgrade" and last_macro["unit"] == unit.name and last_macro["value"] == tostring(unit.current_upgrade - 1) and last_macro["money"] == tostring(Money_Write(unit.name, "Upgrade")) and last_macro["cframe"] == tostring(Unit_CFrame(UpgradeInterface.Name)) then
                                    else
                                        Macro_Insert(
                                            {
                                                ["type"] = "Upgrade",
                        ...
                        Game.Signals[UpgradeInterface.Name.."Sell"] = UpgradeInterface:WaitForChild("Unit"):WaitForChild("Sell"):WaitForChild("Button").InputBegan:Connect(
                            function(input)
                                if (input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch) and Units_Active(UpgradeInterface.Name) and Configs["Macro Record"].Value and not Loader.Unloaded then
                                    ...
                                    Macro_Data_Write()
                                    if last_macro["type"] == "Sell" and last_macro["unit"] == unit.name and last_macro["cframe"] == tostring(Unit_CFrame(UpgradeInterface.Name)) then
                                    else
                                        Macro_Insert(
                                            {
                                                ["type"] = "Sell",
